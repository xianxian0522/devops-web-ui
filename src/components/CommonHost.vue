...
export default {
  ...
  setup(props: any) {
    ...
    const data = ref<Hosts[]>(props.hosts)
    ...
    const debounceName = (value: string) => {
      if (formState.InnerIP) {
        data.value = props.hostList.filter((h: Hosts) => {
          return (h.InnerIP?.indexOf(formState.InnerIP) !== -1 && h.Hostname?.indexOf(value) !== -1)
        })
      } else {
        data.value = props.hostList.filter((h: Hosts) => h.Hostname?.indexOf(value) !== -1)
      }
    }
    const debounceIp = (value: string) => {
      if (formState.Hostname) {
        ...
        data.value = props.hostList.filter((h: Hosts) => h.InnerIP?.indexOf(value) !== -1)
      }
    }
    watch(() => formState.Hostname, _.debounce(debounceName, 300))
    watch(() => formState.InnerIP, _.debounce(debounceIp, 300))

    watch(() => props.hostList, () => {
      data.value = props.hostList
    })
